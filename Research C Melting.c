...
double munSLay(int n,double h, double *r){
  ...
  dn = h*n;
  sum = 0.5*(1.0 + exp(-r2/dn ))/sqrt(dn);
  for (i=1; i<n; i++){
    dn = h*(n-i);
    sum += (1.0 + exp(-r2/dn ))/sqrt(dn);
  }
  sum *= 0.5*M_SQPINV;
...
double munZLay(int n,double h, double *r){
  ...
  dn = h*n;
  sum = 0.5*exp(-r2/(4*dn) )/sqrt(dn);
  for (i=1; i<n; i++){
    dn = h*(n-i);
    sum += exp(-r2/(4*dn) )/sqrt(dn);
  }
  sum *= M_SQPINV;
...
#if 0  
  for (j=1;j<=n;j++){
    qa[j]=-1.0*(sLay(j,dt,rrev,us)+zeroLay(j,dt,rrev,fsrev)+0*munZLay(j,dt,r)*fsrev[j])/(mun(j,dt)*kvtntn());
  }
#endif 
//from the solidification program 

#if 1  
  for (j=1;j<=n;j++){
    qa[j]=(-0.5*us[j]-sLayAdj(j,dt,rrev,qa)-qto(j,dt)*kv(rrev[j],rrev[0],j*dt,0.0)*qa[0]-zeroLay(j,dt,rrev,fsrev))/(munAdj(j,dt)*kvtntn());
  }  
#endif
...
//printf("%10.9f\n",0.9244814067-(zeroLay(numInts,dt,r,fs) + munZLay(numInts,dt,r)*fs[numInts]));
//printf("%10.9f\n",1.234876000-(sLay(numInts,dt,r,q) + munSLay(numInts,dt,r)*q[numInts]));
...
//printf("%10.9f\n",sLay(numInts,dt,r,q)+mun(numInts,dt)*kvtntn()*q[numInts]-0.2584190848);
//printf("%10.9f\n",zeroLay(numInts,dt,r,fs)-0.09929600642);
#endif
}
